...
if LiveDemo
    set(2,'position',[185 372 662 355]);
    changeFig(18,2,12);
else
    fprintf(1,'paused\n');
end
...
if LiveDemo
    pause;
    set(2,'position',[336 527 450 200]);
    changeFig;
    legend('Simulation','Expected PSD',4);
else
    fprintf(1,'paused\n');
...
if LiveDemo
    set(3,'position',[275 365 520 362]);
    changeFig(18,2,12);
else
	fprintf(1,'paused\n');
end
pause
if LiveDemo
    set(3,'position',[815 544 218 183]);
    changeFig;
end
...
for i=1:2
	...
	pause
	if LiveDemo
	    set(1,'position',[8 527 300 250]);
	    changeFig;
	end
	
	f = linspace(0,0.5,N/2+1);
	echo on
	spec = fft(v.*hann(N))/(M*N/4);
	echo off;
	figure(2); 
